...
from fluxory.lib import addrconv
from fluxory.lib.pack_utils import msg_pack_into
from fluxory.lib.packet import packet
from fluxory import exceptions
from fluxory import utils
from fluxory.ofproto.ofproto_parser import StringifyMixin, MsgBase, MsgInMsgBase
from fluxory.ofproto import ether
from fluxory.ofproto import nx_actions
from fluxory.ofproto import ofproto_parser
from fluxory.ofproto import ofproto_common
from fluxory.ofproto import ofproto_v1_5 as ofproto
...
class OFPHello(MsgBase):
    ...
    def __init__(self, elements: None = None) -> None:
        elements = elements if elements else []
        super(OFPHello, self).__init__(ofproto.OFP_VERSION, OFPHello.msg_type)
        self.elements = elements

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPHello, cls).parser(msg_len, xid, buf)
...
class OFPEchoRequest(MsgBase):
    ...
    def __init__(self, data: None = None) -> None:
        super(OFPEchoRequest, self).__init__(ofproto.OFP_VERSION, OFPEchoRequest.msg_type)
        self.data = data

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPEchoRequest, cls).parser(msg_len, xid, buf)
...
class OFPErrorMsg(MsgBase):
    ...
    Types and codes are defined in ``fluxory.ofproto.ofproto``.
    ...
    def __init__(self, type_=None, code=None, data=None, **kwargs):
        super(OFPErrorMsg, self).__init__(ofproto.OFP_VERSION, OFPErrorMsg.msg_type)
        self.type = type_
        self.code = code
    ...
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        ...
        msg = super(OFPErrorMsg, cls).parser(msg_len, xid, buf)
...
class OFPEchoReply(MsgBase):
    ...
    def __init__(self, data=None):
        super(OFPEchoReply, self).__init__(ofproto.OFP_VERSION, OFPEchoReply.msg_type)
        self.data = data

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPEchoReply, cls).parser(msg_len, xid, buf)
        msg.data = msg.buf[ofproto.OFP_HEADER_SIZE:]
        return msg
...
class OFPFeaturesRequest(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_FEATURES_REQUEST

    def __init__(self) -> None:
        super(OFPFeaturesRequest, self).__init__(ofproto.OFP_VERSION, OFPFeaturesRequest.msg_type)
...
class OFPExperimenter(MsgBase):
    ...
    def __init__(self, experimenter=None, exp_type=None, data=None):
        super(OFPExperimenter, self).__init__(ofproto.OFP_VERSION, OFPExperimenter.msg_type)
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPExperimenter, cls).parser(msg_len, xid, buf)
...
class OFPSwitchFeatures(MsgBase):
    ...
    def __init__(self, datapath_id: None = None, n_buffers: None = None,
                 n_tables: None = None, auxiliary_id: None = None, capabilities: None = None) -> None:
        super(OFPSwitchFeatures, self).__init__(ofproto.OFP_VERSION, OFPSwitchFeatures.msg_type)
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPSwitchFeatures, cls).parser(msg_len, xid, buf)
        (msg.datapath_id,
         msg.n_buffers,
         msg.n_tables,
         msg.auxiliary_id,
...
class OFPGetConfigRequest(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_GET_CONFIG_REQUEST

    def __init__(self):
        super(OFPGetConfigRequest, self).__init__(ofproto.OFP_VERSION, OFPGetConfigRequest.msg_type)
...
class OFPGetConfigReply(MsgBase):
    ...
    def __init__(self, flags=None, miss_send_len=None):
        super(OFPGetConfigReply, self).__init__(ofproto.OFP_VERSION, OFPGetConfigReply.msg_type)
        self.flags = flags
        self.miss_send_len = miss_send_len

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPGetConfigReply, cls).parser(msg_len, xid, buf)
...
class OFPSetConfig(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_SET_CONFIG

    def __init__(self, flags=0, miss_send_len=0):
        super(OFPSetConfig, self).__init__(ofproto.OFP_VERSION, OFPSetConfig.msg_type)
        self.flags = flags
        self.miss_send_len = miss_send_len
...
class OFPMatch(StringifyMixin):
    ...
    .. Note::
        ...
        please refer to :ref:`fluxory.ofproto.nx_match <nx_match_structures>`.
...
class OFPPropBase(StringifyMixin):
    ...
    @classmethod
    def register_type(cls, type_: int) -> Callable:
...
class OFPPacketIn(MsgBase):
    ...
    def __init__(self, buffer_id=None, total_len=None, reason=None,
                 table_id=None, cookie=None, match=None, data=None):
        super(OFPPacketIn, self).__init__(ofproto.OFP_VERSION, OFPPacketIn.msg_type)
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPPacketIn, cls).parser(msg_len, xid, buf)
...
class OFPFlowRemoved(MsgBase):
    ...
    def __init__(self, table_id=None, reason=None, priority=None,
        ...
        super(OFPFlowRemoved, self).__init__(ofproto.OFP_VERSION, OFPFlowRemoved.msg_type)
        self.table_id = table_id
        self.reason = reason
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPFlowRemoved, cls).parser(msg_len, xid, buf)
...
def _set_stats_type(stats_type: int, stats_body_cls: Any) -> Callable:
...
class OFPMultipartRequest(MsgBase):
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_MULTIPART_REQUEST

    def __init__(self, flags):
        super(OFPMultipartRequest, self).__init__(ofproto.OFP_VERSION, OFPMultipartRequest.msg_type)
...
class OFPMeterMod(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_METER_MOD

    def __init__(self, command=ofproto.OFPMC_ADD,
                 flags=ofproto.OFPMF_KBPS, meter_id=1, bands=None):
        bands = bands if bands else []
        super(OFPMeterMod, self).__init__(ofproto.OFP_VERSION, OFPMeterMod.msg_type)
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPMeterMod, cls).parser(msg_len, xid, buf)
...
class OFPTableMod(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_TABLE_MOD

    def __init__(self, table_id, config, properties):
        super(OFPTableMod, self).__init__(ofproto.OFP_VERSION, OFPTableMod.msg_type)
        self.table_id = table_id
        self.config = config
        self.properties = properties
...
class OFPMultipartReply(MsgBase):
    ...
    @staticmethod
    def register_stats_type(body_single_struct: bool = False) -> Callable:
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_MULTIPART_REPLY

    def __init__(self, body=None, flags=None):
        super(OFPMultipartReply, self).__init__(ofproto.OFP_VERSION, OFPMultipartReply.msg_type)
        self.body = body
        self.flags = flags
    ...
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        ...
        msg = super(OFPMultipartReply, stats_type_cls).parser(msg_len, xid, buf)
        msg.type = type_
        msg.flags = flags
        ...
        while offset < msg_len:
            ...
            if offset_step < 1:
                raise exceptions.OFPMalformedMessage()
            body.append(b)
            offset += offset_step
...
class OFPDescStatsRequest(OFPMultipartRequest):
    ...
    def __init__(self, flags=0, type_=None):
        super(OFPDescStatsRequest, self).__init__(flags)
...
class OFPDescStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPDescStatsReply, self).__init__(**kwargs)
...
class OFPTableFeaturesStatsRequest(OFPMultipartRequest):
    ...
    def __init__(self, flags=0, body=None, type_=None):
        body = body if body else []
        super(OFPTableFeaturesStatsRequest, self).__init__(flags)
        self.body = body
...
class OFPTableFeaturesStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPTableFeaturesStatsReply, self).__init__(**kwargs)
...
class OFPPortDescStatsRequest(OFPMultipartRequest):
    ...
    def __init__(self, flags=0, port_no=ofproto.OFPP_ANY, type_=None):
        super(OFPPortDescStatsRequest, self).__init__(flags)
        self.port_no = port_no
...
class OFPPortDescStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPPortDescStatsReply, self).__init__(**kwargs)
...
class OFPTableDescStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_table_desc_stats_request(self):
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, type_=None):
        super(OFPTableDescStatsRequest, self).__init__(flags)
...
class OFPTableDescStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPTableDescStatsReply, self).__init__(**kwargs)
...
class OFPQueueDescStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_queue_desc_stats_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, port_no=ofproto.OFPP_ANY,
                 queue_id=ofproto.OFPQ_ALL, type_=None):
        super(OFPQueueDescStatsRequest, self).__init__(flags)
        self.port_no = port_no
        self.queue_id = queue_id
...
class OFPQueueDescStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPQueueDescStatsReply, self).__init__(**kwargs)
...
class OFPQueueStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_queue_stats_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, port_no=ofproto.OFPP_ANY,
                 queue_id=ofproto.OFPQ_ALL, type_=None):
        super(OFPQueueStatsRequest, self).__init__(flags)
        self.port_no = port_no
        self.queue_id = queue_id
...
class OFPQueueStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPQueueStatsReply, self).__init__(**kwargs)
...
class OFPGroupStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_group_stats_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, group_id=ofproto.OFPG_ALL,
                 type_=None):
        super(OFPGroupStatsRequest, self).__init__(flags)
        self.group_id = group_id
...
class OFPGroupStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPGroupStatsReply, self).__init__(**kwargs)
...
class OFPGroupDescStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_group_desc_stats_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, group_id=ofproto.OFPG_ALL,
                 type_=None):
        super(OFPGroupDescStatsRequest, self).__init__(flags)
        self.group_id = group_id
...
class OFPGroupDescStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPGroupDescStatsReply, self).__init__(**kwargs)
...
class OFPGroupFeaturesStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_group_features_stats_request(self):
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, type_=None):
        super(OFPGroupFeaturesStatsRequest, self).__init__(flags)
...
class OFPGroupFeaturesStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPGroupFeaturesStatsReply, self).__init__(**kwargs)
...
class OFPMeterStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_meter_stats_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, meter_id=ofproto.OFPM_ALL,
                 type_=None):
        super(OFPMeterStatsRequest, self).__init__(flags)
        self.meter_id = meter_id
...
class OFPMeterStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPMeterStatsReply, self).__init__(**kwargs)
...
class OFPMeterBandHeader(OFPMeterBand):
    ...
    @staticmethod
    def register_meter_band_type(type_: int, len_: int) -> Callable:
...
class OFPMeterDescStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_meter_desc_stats_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, meter_id=ofproto.OFPM_ALL,
                 type_=None):
        super(OFPMeterDescStatsRequest, self).__init__(flags)
        self.meter_id = meter_id
...
class OFPMeterDescStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPMeterDescStatsReply, self).__init__(**kwargs)
...
class OFPMeterFeaturesStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_meter_features_stats_request(self):
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, type_=None):
        super(OFPMeterFeaturesStatsRequest, self).__init__(flags)
...
class OFPMeterFeaturesStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPMeterFeaturesStatsReply, self).__init__(**kwargs)
...
class OFPFlowUpdateHeader(OFPFlowUpdate):
    ...
    @staticmethod
    def register_flow_update_event(event: int, length: int) -> Callable:
...
class OFPFlowMonitorRequestBase(OFPMultipartRequest):
    def __init__(self, flags, monitor_id, out_port, out_group,
                 monitor_flags, table_id, command, match):
        super(OFPFlowMonitorRequestBase, self).__init__(flags)
...
class OFPFlowMonitorRequest(OFPFlowMonitorRequestBase):
    ...
    Example::
        ...
        def send_flow_monitor_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
...
class OFPFlowMonitorReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPFlowMonitorReply, self).__init__(**kwargs)
...
class OFPBundleFeaturesStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_bundle_features_stats_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, feature_request_flags=0,
                 properties=None, type_=None):
        properties = properties if properties else []
        super(OFPBundleFeaturesStatsRequest, self).__init__(flags)
...
class OFPBundleFeaturesStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPBundleFeaturesStatsReply, self).__init__(**kwargs)
...
class OFPExperimenterStatsRequestBase(OFPMultipartRequest):
    def __init__(self, flags,
                 ...
                 type_=None):
        super(OFPExperimenterStatsRequestBase, self).__init__(flags)
        self.experimenter = experimenter
        self.exp_type = exp_type
...
class OFPExperimenterStatsRequest(OFPExperimenterStatsRequestBase):
    ...
    def __init__(self, flags,
                 experimenter, exp_type, data,
                 type_=None):
        super(OFPExperimenterStatsRequest, self).__init__(flags,
                                                          experimenter,
                                                          exp_type, type_)
        self.data = data
...
class OFPExperimenterStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPExperimenterStatsReply, self).__init__(**kwargs)
...
class OFPFlowStatsRequestBase(OFPMultipartRequest):
    def __init__(self, flags, table_id, out_port, out_group,
                 cookie, cookie_mask, match):
        super(OFPFlowStatsRequestBase, self).__init__(flags)
        self.table_id = table_id
        self.out_port = out_port
        self.out_group = out_group
        self.cookie = cookie
...
class OFPFlowDescStatsRequest(OFPFlowStatsRequestBase):
    ...
    Example::
        ...
        def send_flow_desc_request(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, table_id=ofproto.OFPTT_ALL,
        ...
        super(OFPFlowDescStatsRequest, self).__init__(
            flags, table_id, out_port, out_group, cookie,
            cookie_mask, match)
...
class OFPFlowDescStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPFlowDescStatsReply, self).__init__(**kwargs)
...
class OFPFlowStatsRequest(OFPFlowStatsRequestBase):
    ...
    def __init__(self, flags=0, table_id=ofproto.OFPTT_ALL,
        ...
        if match is None:
            match = OFPMatch()
        super(OFPFlowStatsRequest, self).__init__(flags, table_id,
                                                  out_port, out_group,
                                                  cookie, cookie_mask, match)
...
class OFPFlowStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPFlowStatsReply, self).__init__(**kwargs)
...
class OFPAggregateStatsRequest(OFPFlowStatsRequestBase):
    ...
    def __init__(self, flags, table_id, out_port, out_group,
        ...
        super(OFPAggregateStatsRequest, self).__init__(flags,
                                                       table_id,
                                                       out_port,
                                                       out_group,
                                                       cookie,
...
class OFPAggregateStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPAggregateStatsReply, self).__init__(**kwargs)
...
class OFPTableStatsRequest(OFPMultipartRequest):
    ...
    def __init__(self, flags, type_=None):
        super(OFPTableStatsRequest, self).__init__(flags)
...
class OFPTableStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPTableStatsReply, self).__init__(**kwargs)
...
class OFPPortStatsRequest(OFPMultipartRequest):
    ...
    def __init__(self, flags, port_no, type_=None):
        super(OFPPortStatsRequest, self).__init__(flags)
        self.port_no = port_no
...
class OFPPortStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPPortStatsReply, self).__init__(**kwargs)
...
class OFPBarrierRequest(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_BARRIER_REQUEST

    def __init__(self):
        super(OFPBarrierRequest, self).__init__(ofproto.OFP_VERSION, OFPBarrierRequest.msg_type)
...
class OFPBarrierReply(MsgBase):
    ...
    def __init__(self):
        super(OFPBarrierReply, self).__init__(ofproto.OFP_VERSION, OFPBarrierReply.msg_type)
...
class OFPPortStatus(MsgBase):
    ...
    def __init__(self, reason=None, desc=None):
        super(OFPPortStatus, self).__init__(ofproto.OFP_VERSION, OFPPortStatus.msg_type)
        self.reason = reason
        self.desc = desc

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPPortStatus, cls).parser(msg_len, xid, buf)
...
class OFPRoleStatus(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_ROLE_STATUS

    def __init__(self, role=None, reason=None,
                 generation_id=None, properties=None):
        super(OFPRoleStatus, self).__init__(ofproto.OFP_VERSION, OFPRoleStatus.msg_type)
        self.role = role
        self.reason = reason
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPRoleStatus, cls).parser(msg_len, xid, buf)
...
class OFPTableStatus(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_TABLE_STATUS

    def __init__(self, reason=None, table=None):
        super(OFPTableStatus, self).__init__(ofproto.OFP_VERSION, OFPTableStatus.msg_type)
        self.reason = reason
        self.table = table

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPTableStatus, cls).parser(msg_len, xid, buf)
...
class OFPRequestForward(MsgInMsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_REQUESTFORWARD

    def __init__(self, request=None):
        super(OFPRequestForward, self).__init__(ofproto.OFP_VERSION, OFPRequestForward.msg_type)
        self.request = request

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPRequestForward, cls).parser(msg_len, xid, buf)
        req_buf = buf[ofproto.OFP_HEADER_SIZE:]
        (_ver, _type, _len, _xid) = ofproto_parser.header(req_buf)
        msg.request = ofproto_parser.msg(
            _ver, _type, _len, _xid, req_buf)
        return msg
...
class OFPControllerStatusStatsRequest(OFPMultipartRequest):
    ...
    Example::
        ...
        def send_controller_status_multipart_request(self):
            ofp_parser = datapath.ofproto_parser
    ...
    def __init__(self, flags=0, type_=None):
        super(OFPControllerStatusStatsRequest, self).__init__(flags)
...
class OFPControllerStatusStatsReply(OFPMultipartReply):
    ...
    def __init__(self, type_=None, **kwargs):
        super(OFPControllerStatusStatsReply, self).__init__(**kwargs)
...
class OFPControllerStatus(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_CONTROLLER_STATUS

    def __init__(self, status=None):
        super(OFPControllerStatus, self).__init__(ofproto.OFP_VERSION, OFPControllerStatus.msg_type)
        self.status = status

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPControllerStatus, cls).parser(msg_len,
                                                     xid, buf)
...
class OFPPacketOut(MsgBase):
    ...
    Example::
        ...
        def send_packet_out(self, buffer_id, in_port):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_PACKET_OUT

    def __init__(self, buffer_id=None, match=None, actions=None,
                 data=None, actions_len=None):
        super(OFPPacketOut, self).__init__(ofproto.OFP_VERSION, OFPPacketOut.msg_type)
...
class OFPFlowMod(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_FLOW_MOD

    def __init__(self, cookie=0, cookie_mask=0, table_id=0,
                 ...
                 instructions=None):
        instructions = instructions if instructions else []
        super(OFPFlowMod, self).__init__(ofproto.OFP_VERSION, OFPFlowMod.msg_type)
        self.cookie = cookie
        self.cookie_mask = cookie_mask
        self.table_id = table_id
        self.command = command
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPFlowMod, cls).parser(msg_len, xid, buf)
...
class OFPInstruction(StringifyMixin):
    ...
    @staticmethod
    def register_instruction_type(types: List[int]) -> Callable:
...
class OFPAction(OFPActionHeader):
    ...
    @staticmethod
    def register_action_type(type_: int, len_: int) -> Callable:
...
class OFPActionExperimenter(OFPAction):
    ...
    .. Note::
        ...
        please refer to :ref:`fluxory.ofproto.nx_actions <nx_actions_structures>`.
    """
...
class OFPGroupMod(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_GROUP_MOD

    def __init__(self, command=ofproto.OFPGC_ADD,
        ...
        buckets = buckets if buckets else []
        properties = properties if properties else []
        super(OFPGroupMod, self).__init__(ofproto.OFP_VERSION, OFPGroupMod.msg_type)
        self.command = command
        self.type = type_
        self.group_id = group_id
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPGroupMod, cls).parser(msg_len, xid, buf)
...
class OFPPortMod(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_PORT_MOD

    def __init__(self, port_no=0, hw_addr='00:00:00:00:00:00',
                 config=0, mask=0, properties=None):
        super(OFPPortMod, self).__init__(ofproto.OFP_VERSION, OFPPortMod.msg_type)
        self.port_no = port_no
        self.hw_addr = hw_addr
        self.config = config
        self.mask = mask
...
class OFPRoleRequest(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_ROLE_REQUEST

    def __init__(self, role=None, short_id=None,
                 generation_id=None):
        super(OFPRoleRequest, self).__init__(ofproto.OFP_VERSION, OFPRoleRequest.msg_type)
        self.role = role
        self.short_id = short_id
        self.generation_id = generation_id
...
class OFPRoleReply(MsgBase):
    ...
    def __init__(self, role=None, short_id=None,
                 generation_id=None):
        super(OFPRoleReply, self).__init__(ofproto.OFP_VERSION, OFPRoleReply.msg_type)
        self.role = role
        self.short_id = short_id
        self.generation_id = generation_id
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPRoleReply, cls).parser(msg_len, xid, buf)
...
class OFPGetAsyncRequest(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_GET_ASYNC_REQUEST

    def __init__(self):
        super(OFPGetAsyncRequest, self).__init__(ofproto.OFP_VERSION, OFPGetAsyncRequest.msg_type)
...
class OFPGetAsyncReply(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_GET_ASYNC_REPLY

    def __init__(self, properties=None):
        super(OFPGetAsyncReply, self).__init__(ofproto.OFP_VERSION, OFPGetAsyncReply.msg_type)
        self.properties = properties

    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPGetAsyncReply, cls).parser(msg_len, xid, buf)
...
class OFPSetAsync(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_SET_ASYNC

    def __init__(self, properties=None):
        super(OFPSetAsync, self).__init__(ofproto.OFP_VERSION, OFPSetAsync.msg_type)
        self.properties = properties
...
class OFPBundleCtrlMsg(MsgBase):
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_BUNDLE_CONTROL

    def __init__(self, bundle_id=None, type_=None, flags=None,
                 properties=None):
        super(OFPBundleCtrlMsg, self).__init__(ofproto.OFP_VERSION, OFPBundleCtrlMsg.msg_type)
    ...
    @classmethod
    def parser(cls, msg_len: int, xid: int, buf: bytes) -> Callable:
        msg = super(OFPBundleCtrlMsg, cls).parser(msg_len,
                                                  xid, buf)
...
class OFPBundleAddMsg(MsgInMsgBase):
    ...
    Example::
        ...
        def send_bundle_add_message(self):
            ofp = datapath.ofproto
            ofp_parser = datapath.ofproto_parser
    ...
    version = ofproto.OFP_VERSION
    msg_type = ofproto.OFPT_BUNDLE_ADD_MESSAGE

    def __init__(self, bundle_id, flags, message, properties):
        super(OFPBundleAddMsg, self).__init__(ofproto.OFP_VERSION, OFPBundleAddMsg.msg_type)
        self.bundle_id = bundle_id
        self.flags = flags
...
nx_actions.generate(
    'fluxory.ofproto.ofproto_v1_5',
    'fluxory.ofproto.ofproto_v1_5_parser'
)


_classes: Dict[int, Callable] = {}


def _register_classes() -> None:
    clsmembers = inspect.getmembers(sys.modules[__name__], inspect.isclass)
    for cls in clsmembers:
        msg_type = getattr(cls[1], 'msg_type', -1)
        if msg_type > -1:
            if msg_type not in _classes:
                _classes[msg_type] = cls[1]


_register_classes()


def from_type(msg_type: int, *args, **kwargs) -> None:
    """Get class from msg_type."""
    try:
        return _classes[msg_type]
    except KeyError:
        raise FluxoryError("Unsupported msg_type {msg_type}")

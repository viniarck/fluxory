...
from fluxory.ofproto import nicira_ext
from fluxory.ofproto import ofproto_utils
from fluxory.ofproto import oxm_fields
from fluxory.ofproto import oxs_fields
...
def _oxm_tlv_header(class_: int, field: int, hasmask: int, length: int) -> int:
    return (class_ << 16) | (field << 9) | (hasmask << 8) | length


def oxm_tlv_header(field: int, length: int) -> int:
    return _oxm_tlv_header(OFPXMC_OPENFLOW_BASIC, field, 0, length)


def oxm_tlv_header_w(field: int, length: int) -> int:
...
def _oxs_tlv_header(class_: int, field: int, reserved: int, length: int) -> int:
    return (class_ << 16) | (field << 9) | (reserved << 8) | length


def oxs_tlv_header(field: int, length: int) -> int:
